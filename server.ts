...
async function initDb() {
  ...
  const seedData = async () => {
    ...
    for (const [nombre, precio] of actividadesList) {
      await pool.query("INSERT INTO actividades (nombre, precio) VALUES ($1, $2) ON CONFLICT (nombre) DO NOTHING", [nombre.trim().toUpperCase(), precio]);
    }
...
async function startServer() {
  ...
  app.post("/api/destajistas", async (req, res) => {
    ...
    try {
      const { rows } = await pool.query("INSERT INTO destajistas (nombre) VALUES ($1) RETURNING id", [normalizedNombre]);
  ...
  app.post("/api/actividades", async (req, res) => {
    ...
    const normalizedNombre = nombre.trim().toUpperCase();
    try {
      const { rows } = await pool.query("INSERT INTO actividades (nombre, precio) VALUES ($1, $2) RETURNING id", [normalizedNombre, precio]);
      ...
      if (Array.isArray(data)) {
        ...
        try {
          ...
          for (const item of data) {
            await client.query("INSERT INTO ubicaciones (paquete, manzana, lote) VALUES ($1, $2, $3) ON CONFLICT (paquete, manzana, lote) DO NOTHING", 
              [item.paquete, item.manzana, item.lote]);
          }
          await client.query('COMMIT');
        ...
        const { rows } = await pool.query("INSERT INTO ubicaciones (paquete, manzana, lote) VALUES ($1, $2, $3) ON CONFLICT (paquete, manzana, lote) DO NOTHING RETURNING id", 
          [paquete, manzana, lote]);
        io.emit("data_changed", { type: "ubicaciones" });
        res.json({ id: rows[0]?.id });
      }
    } catch (e: any) {
  ...
  app.get("/api/capturas", async (req, res) => {
    ...
    if (semana) {
      query += ` AND c.semana = $${params.length + 1}`;
      params.push(semana);
    }
    if (destajista_id) {
      query += ` AND c.destajista_id = $${params.length + 1}`;
  ...
  app.post("/api/capturas", async (req, res) => {
    ...
    try {
      const checkDuplicate = async (client: any, destajista_id: number, actividad_id: number, paquete: string, manzana: string, lotes: string) => {
